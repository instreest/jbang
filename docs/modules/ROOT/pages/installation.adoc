...
- **Windows (CMD)**: `jbang.cmd` delegates to `jbang.ps1`, so the same retry logic applies
...
JBang provides tab completion for bash, zsh, fish, and PowerShell (experimental).
The shell is auto-detected, so in most cases you just need to run:

[source,bash]
----
source <(jbang completion)     # bash/zsh
jbang completion | source       # fish
(jbang completion) -join "`n" | Invoke-Expression  # pwsh
----

To make it permanent, pick the approach for your shell:
...
Or save to your fpath (ensure the directory is in `fpath` before `compinit`):
...
[source,powershell]
----
(jbang completion) -join "`n" | Invoke-Expression
----
...
# Add to $PROFILE:
. ~/jbang-completion.ps1
----

TIP: You can explicitly specify the shell with `jbang completion <shell>` (e.g., `jbang completion fish`) if auto-detection does not work.
...
| `17`
| Java version to install when no JDK is found on the system. On Windows the launchers prefer the JDK selected with `jbang jdk default` and the JDK of this version that JBang downloaded itself; `JAVA_HOME` is only used when neither exists and it points to a JDK of at least this version (as read from its `release` file). JDKs found on the `PATH` are not used.
