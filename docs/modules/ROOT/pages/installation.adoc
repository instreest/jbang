...
| `17`
| Java version to install when no JDK is found on the system. On Windows the launchers prefer the JDK selected with `jbang jdk default` and only use `JAVA_HOME` when it points to a JDK of at least this version (as read from its `release` file); JDKs found on the `PATH` are not used.
