...
- Startup scripts live in `src/main/scripts/`: `jbang` (bash), `jbang.cmd` (CMD), `jbang.ps1` (PowerShell). `jbang.cmd` only runs JBang directly when the native binary, or a JDK and `jbang.jar`, are already available; everything else (downloads, JDK installs, staged updates) is delegated to `jbang.ps1` (run with `JBANG_RUNTIME_SHELL=cmd`, in which case `jbang.ps1` prints the command JBang asks to execute instead of running it). `jbang.ps1` is laid out as configuration, then functions, then the main flow; functions read that configuration but take everything else as a parameter. Keep Windows logic in `jbang.ps1` and keep the JDK lookup order in `jbang.cmd` in sync with it (`currentjdk`, then the cached default JDK, then a JAVA_HOME of at least `JBANG_DEFAULT_JAVA_VERSION`; the PATH is not consulted on Windows). Changes affecting downloads or bootstrap must be applied consistently to `jbang` and `jbang.ps1`. Behavior (e.g., retry backoff) must be consistent across tools — watch for tool-specific quirks like `curl --retry-delay 0` meaning exponential backoff while `wget --waitretry=0` meaning no delay.
- Environment variables follow `JBANG_*` naming. New env vars must have defaults in each script that actually uses them (`jbang.cmd` delegates downloads and JDK installs to `jbang.ps1`, so download-related vars only need defaults in `jbang` and `jbang.ps1`). Document new vars in `installation.adoc` ("Startup Script Environment Variables") and ensure consistent behavior across platforms.
